...
<div id="main" class="container-fluid">
 ...
 -->
		...
		<?php 
				if(isset($_POST['domainName']) and !empty($_POST['domainName'])){
					$dm = $_POST['domainName'];
						$sql = "SELECT new_website.website_id FROM `new_website` where new_website.domain_name = '$dm'";
						$qry=mysql_query($sql) or die(mysql_error());
								if(mysql_num_rows($qry)){
									
									while($row = mysql_fetch_row($qry)){
										//echo "<script>alert('".$row[0].$dm."')</script>";
									$GLOBALS["id"] = $row[0];
					}
				}
			}
	...
	<div class="links">
		...
		<a class="openpop" href="http://localhost/mysafehome/safehome/created_site_data.php?id=<?php echo $GLOBALS['id'];?>" target="_blank" style = "display:<?php if(!empty($GLOBALS['id'])){echo 'block';}else{echo 'none';} ?>">Click to go New site</a>
		
		</div>
	
		<div class="wrapper">

		    <div class="popup" style="position:absolute; top:0px;left:0%;width:100%;height:650px;z-index:3000px">
		     <a href="#" class="close" style="position:absolute; top:0px; right:0px;">
		         <button class="btn-danger">X</button></a><br>
		        <iframe src="" style="position:absolute; top:25px;left:12px;width:99%;height: 100%;">
...
<button onclick="myFunction(<?php echo $GLOBALS['id'];?>)" style = "display:<?php if(!empty($GLOBALS['id'])){echo 'block';}else{echo 'none';} ?>">Go to Your site</button>

<script>
function myFunction(idno) {
		alert(idno);
    var myWindow = window.open("http://localhost/mysafehome/safehome/created_site_data.php?id="+idno, "", "location=no, top = 5,left=310,width=1200,height=740");
}
</script>

			<!-- iframe popup box end-->
