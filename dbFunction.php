...
$sql=mysql_query("select * from safehome_user where email_id='".$email."' AND user_pass='".$password."'");	
...
return 1;
 ...
 public function  facelity_limit($strInd,$limit)
 {
 $qry=mysql_query("select * from facility_booking limit $limit offset $strInd") or die(mysql_error());
 $row=mysql_num_rows($qry);
 if($row>0)
 {
 return $qry;
...
//data getting idex based
public function  get_complaints_index($crntInd,$limit )
{

	if($qry=mysql_query("select * from complaint limit $limit offset $crntInd") or die(mysql_error()))
{
return $qry;
}
